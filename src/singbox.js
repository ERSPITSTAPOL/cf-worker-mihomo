...
function loadAndSetOutbounds(Outbounds, ApiUrlname) {
    ...
    const updateOutboundsArray = (outbound, matchedOutbounds) => {
        if (matchedOutbounds.length > 0) outbound.outbounds = outbound.outbounds ? [...new Set([...outbound.outbounds, ...matchedOutbounds])] : matchedOutbounds;
        delete outbound.filter;
        return outbound;
    };

    const cleanRemovedTags = (outbounds) => {
        const removedTags = outbounds.filter((item) => !item.outbounds || (Array.isArray(item.outbounds) && item.outbounds.length === 0))
            .map((item) => item.tag)
